		...
		printer.nav.goto('frm/getPhotosFromServerAutoPrint.html', function (data) { //页面流转
			...
			printer.normal.getFileByCode(printObj.imgUrl, function (err, stdout, stderr) {
				...
				if (err) {
					...
					$('#getedPhoto').
						empty().
						append('<img class="ani_in"  ondragstart="return false;" src="public/photo/' + printObj.imgUrl + '.jpg">');
					//调用打印机
					setTimeout(function(){
...
var CountDown=function() {
	var api=printer.appConfig.api.server;
	var cmd=api + "/api/clientInfo/" + printer.appConfig.sevice.machineNo;
	$.get(cmd, function(){
		var a = document.getElementById("CountDownShow");
		if(a)
		{
